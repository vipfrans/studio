...
export const LastWinnings = () => {
  ...
  useEffect(() => {
    ...
    const generateSimWinning = () => {
      const users = ['Frosty', 'Lumine', 'VoidX', 'Ghost', 'Stellar', 'Rex', 'Kone', 'Valk', 'Vortex', 'Pulse', 'Ace', 'Neo', 'Storm'];
      const games = ['Rocket', 'Mines', 'Coinflip', 'Cases'] as const;
      const newUser = users[Math.floor(Math.random() * users.length)];
      const game = games[Math.floor(Math.random() * games.length)];
      const amount = Math.floor(Math.random() * 5000) + 20;
    ...
    const interval = setInterval(generateSimWinning, getInterval());
          ...
          {winnings.map((win) => {
            ...
            return (
              <motion.div
                ...
                className={`flex-shrink-0 flex items-center gap-2 sm:gap-3 bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl px-3 sm:px-5 py-1.5 sm:py-2 hover:bg-white/10 transition-colors cursor-default ${win.isReal ? 'border-primary/50 bg-primary/5 shadow-[0_0_15px_rgba(200,153,255,0.3)]' : ''}`}
              >
