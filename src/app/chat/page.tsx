    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-12 h-[calc(100vh-80px)] sm:h-[85vh] flex flex-col pb-24 sm:pb-32">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="px-4 py-2 bg-primary/10 rounded-full border border-primary/20 flex items-center gap-2">
          ...
          <span className="text-[10px] font-black text-primary uppercase">Real-time chat active</span>
        </div>
      </div>
      ...
      <div className="flex-1 glass-purple rounded-[32px] border-2 border-primary/20 flex flex-col overflow-hidden">
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 no-scrollbar">
          {messages.map((msg: any) => (
            ...
            >
              <div className={`flex items-center gap-2 mb-1 ${msg.userId === userProfile?.uid ? 'flex-row-reverse' : 'flex-row'}`}>
                {renderRoleBadge(msg.role)}
                <span className="text-[10px] font-bold text-muted-foreground">{msg.username}</span>
                <button onClick={() => setReplyingTo(msg)} className="opacity-0 group-hover:opacity-100 p-1 text-primary">
